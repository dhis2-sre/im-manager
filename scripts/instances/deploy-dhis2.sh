...
ENABLE_PERSISTENT_STORAGE=${ENABLE_PERSISTENT_STORAGE:-true}
PERSISTENT_STORAGE_VOLUME_SIZE=${PERSISTENT_STORAGE_VOLUME_SIZE:-1Gi}
...
echo "{
  ...
  \"parameters\": {
    ...
    \"ENABLE_PERSISTENT_STORAGE\": {
      \"value\": \"$ENABLE_PERSISTENT_STORAGE\"
    },
    \"PERSISTENT_STORAGE_VOLUME_SIZE\": {
      \"value\": \"$PERSISTENT_STORAGE_VOLUME_SIZE\"
    }
  }
}" | $HTTP post "$IM_HOST/deployments/$DEPLOYMENT_ID/instance" "Authorization: Bearer $ACCESS_TOKEN"
