...
def get_deployments_by_environment(access_token: str, im_host: str, environment: str, cluster_map: Dict[int, Cluster]) -> List[Deployment]:
    ...
    for group_data in groups_data:
        for deployment_data in group_data["deployments"]:
            group_id = deployment_data["group"].get("id") or group_data.get("id")
            if group_id is None:
                print(
                    f"Error: Missing group.id for deployment '{deployment_data['name']}' "
                    f"in {environment} from {im_host}/deployments"
                )
                sys.exit(1)
