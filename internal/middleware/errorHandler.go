...
func ErrorHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if errdef.IsBadRequest(err) {
			...
			var errBody string
			if id, ok := GetRequestID(c); ok {
				errBody = fmt.Sprintf("something went wrong. We'll look into it if you send us the request id %q :)", id)
			} else {
				errBody = "something went wrong."
			}
			c.String(http.StatusInternalServerError, errBody)
		}
	}
}
