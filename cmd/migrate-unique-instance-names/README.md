...
### Phase 6 (optional): Apply migration
...
You will reset deployments and verify connectivity after cleaning up orphaned Helm releases (Phase 6c).
...
### Phase 6c: Reset deployments and verify

After the migration is applied (Phase 6) and the old Helm releases are removed (Phase 6b), **reset** all deployments in the group so instances are recreated with the new pod names.

From `scripts/instances` (with `$GROUP` and auth set):
