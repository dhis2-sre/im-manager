...
var DHIS2V2 = withGroupedParameters(Stack{
	...
	ParameterGroups: []ParameterGroup{
		{Name: "dhis2", Title: "DHIS 2 Core", Parameters: StackParameters{
			...
			"CORE_RESOURCES_REQUESTS_CPU":     {Priority: 20, DisplayName: "Resources Requests CPU", DefaultValue: &coreDefaults.resourcesRequestsCPU},
			"CORE_RESOURCES_REQUESTS_MEMORY":  {Priority: 21, DisplayName: "Resources Requests Memory", DefaultValue: &coreDefaults.resourcesRequestsMemory},
			"MIN_READY_SECONDS":               {Priority: 24, DisplayName: "Minimum Ready Seconds", DefaultValue: &coreDefaults.minReadySeconds},
			"LIVENESS_PROBE_TIMEOUT_SECONDS":  {Priority: 25, DisplayName: "Liveness Probe Timeout Seconds", DefaultValue: &coreDefaults.livenessProbeTimeoutSeconds},
			"READINESS_PROBE_TIMEOUT_SECONDS": {Priority: 26, DisplayName: "Readiness Probe Timeout Seconds", DefaultValue: &coreDefaults.readinessProbeTimeoutSeconds},
			"STARTUP_PROBE_FAILURE_THRESHOLD": {Priority: 27, DisplayName: "Startup Probe Failure Threshold", DefaultValue: &coreDefaults.startupProbeFailureThreshold},
			"STARTUP_PROBE_PERIOD_SECONDS":    {Priority: 28, DisplayName: "Startup Probe Period Seconds", DefaultValue: &coreDefaults.startupProbePeriodSeconds},
			"CHART_VERSION":                   {Priority: 29, DisplayName: "Chart Version", DefaultValue: &dhis2V2Defaults.chartVersion},
			"JAVA_OPTS":                       {Priority: 30, DisplayName: "JAVA Options", DefaultValue: &coreDefaults.javaOpts},
			"ENABLE_QUERY_LOGGING":            {Priority: 31, DisplayName: "Enable Query Logging", DefaultValue: &coreDefaults.enableQueryLogging},
			"SAME_SITE_COOKIES":               {Priority: 32, DisplayName: "Same site cookies", DefaultValue: &coreDefaults.sameSiteCookies, Validator: sameSiteCookies},
			"CUSTOM_DHIS2_CONFIG":             {Priority: 33, DisplayName: "Custom DHIS2 config (applied to top of dhis.conf)", DefaultValue: &coreDefaults.customDhis2Config, Sensitive: true},
			"GOOGLE_AUTH_PROJECT_ID":          {Priority: 34, DisplayName: "Google auth project id", DefaultValue: &coreDefaults.googleAuthProjectId, Sensitive: true},
			"GOOGLE_AUTH_PRIVATE_KEY":         {Priority: 35, DisplayName: "Google auth private key", DefaultValue: &coreDefaults.googleAuthPrivateKey, Sensitive: true},
			"GOOGLE_AUTH_PRIVATE_KEY_ID":      {Priority: 36, DisplayName: "Google auth private key id", DefaultValue: &coreDefaults.googleAuthPrivateKeyId, Sensitive: true},
			"GOOGLE_AUTH_CLIENT_EMAIL":        {Priority: 37, DisplayName: "Google auth client email", DefaultValue: &coreDefaults.googleAuthClientEmail, Sensitive: true},
		...
		{Name: "db", Title: "PostgreSQL", Parameters: StackParameters{
			...
			"DATABASE_USERNAME":            {Priority: 8, DisplayName: "Database Username", DefaultValue: &dbDefaults.dbUsername, Sensitive: true},
			"DB_RESOURCES_REQUESTS_CPU":    {Priority: 22, DisplayName: "Resources Requests CPU", DefaultValue: &dbDefaults.resourcesRequestsCPU},
			"DB_RESOURCES_REQUESTS_MEMORY": {Priority: 23, DisplayName: "Resources Requests Memory", DefaultValue: &dbDefaults.resourcesRequestsMemory},
		}},
		...
		{Name: "s3", Title: "Storage: S3", When: whenStorageIsS3, Parameters: StackParameters{
			"S3_BUCKET":   {Priority: 13, DisplayName: "Bucket", DefaultValue: &coreDefaults.s3Bucket},
			"S3_REGION":   {Priority: 14, DisplayName: "Region", DefaultValue: &coreDefaults.s3Region, Sensitive: true},
			"S3_IDENTITY": {Priority: 15, DisplayName: "Identity", DefaultValue: &coreDefaults.s3Identity, Sensitive: true},
			"S3_SECRET":   {Priority: 16, DisplayName: "Secret", DefaultValue: &coreDefaults.s3Secret, Sensitive: true},
		}},
